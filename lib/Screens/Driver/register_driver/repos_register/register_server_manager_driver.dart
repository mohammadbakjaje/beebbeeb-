import 'dart:convert';

import 'package:http/http.dart' as http;
import 'package:untitled1/Screens/register/model/register_model.dart';

import '../../../../helper/constants.dart';
import '../../../../helper/local_network.dart';

class DriverRegisterServerManager {
  Future<RegisterModel> registerDriver(String firstName, String lastName,
      String phone, String password, String confirmPassword) async {
    print("hello");
    final http.Response response = await http.post(
      Uri.parse(
          '$BaseUrl/driver/register?name=$firstName $lastName&phone=$phone&password=$password&password_confirmation=$confirmPassword'),
    );
    print(response.body);
    Map<String, dynamic> data = jsonDecode(response.body);
    if (response.statusCode == 200 && data["status"] == true) {
      await CacheNetwork.insertToCache(
          key: "driver_token", value: data["token"]);
      return RegisterModel.fromJson(data);
    } else {
      throw (data["msg"]);
    }
  }
}
